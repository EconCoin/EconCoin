...
const string strMessageMagic = "EconCoin Signed Message:\n";
...
{
    int64 nSubsidy = 25000 * COIN;


    // Subsidy is cut in half every 524160 blocks, which will occur approximately every 1/2 Year.
    nSubsidy >>= (nHeight / 524160); // EconCoin: 1 Block/30 Seconds.  20160 Blocks/ Week, and 524160 Blocks/ 26 Weeks

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.35 * 24 * 60 * 60; // EconCoin: 0.35 days
static const int64 nTargetSpacing = 30; // EconCoin: 15 seconds
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal EconCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "EconCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
	...
	// vMerkleTree: 5a2e19825b
        ...
        // Genesis block
        const char* pszTimestamp = "EconCoin, created January 26, 2014.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
